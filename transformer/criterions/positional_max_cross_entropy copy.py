...
class PositionalMaxCrossEntropyCriterionConfig(FairseqDataclass):
    ...
    max_loss:float=field(default=3, metadata={"help": "Maximum loss value for the weights"})
...
class PositionalMaxCrossEntropyCriterion(FairseqCriterion):
    def __init__(self, task, sentence_avg, size,report_original_loss,init_decay_rate,init_min_loss,min_loss,max_loss):
        ...
        self.weights=torch.nn.Parameter( self.build_weights(size,init_decay_rate,init_min_loss),requires_grad=True)
    ...
    def build_weights(self, size, decay_rate, min_loss):
        return torch.tensor([1. for i in range(size)])
    # @staticmethod
    # def maximize_loss( module, grad_input, grad_output):
    #     # print(module)
    #     return tuple(-g for g in grad_input) if grad_input else None
